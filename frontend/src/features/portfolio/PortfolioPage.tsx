...
type PortfolioPageProps = { profile: Profile; onBack: () => void; onBooking: () => void }

export function PortfolioPage({ profile, onBack, onBooking }: PortfolioPageProps) {
  ...
  return <section className={styles.page}>
    ...
    <header className={styles.hero}><div className={styles.profileImage}>{profile.imageUrl ? <img src={profile.imageUrl} alt="" style={{ objectPosition: profile.imagePosition ?? '50% 50%' }} /> : profile.name.split(' ').map((name) => name[0]).join('').slice(0, 2)}</div><div><p className="eyebrow">{profile.role}</p><h1>{profile.name}</h1><p>{profile.description}</p><button className={styles.bookingCta} type="button" onClick={onBooking}>Agendar este artista →</button></div></header>
