...
| Desktop UX: vista dos conteúdos por artista | TODO | O utilizador reportou que a apresentação dos conteúdos de cada artista no computador está incorreta. Auditar a vista de portfólio/conteúdos por perfil em desktop (layout, cartões, dimensões, alinhamento e visualização de media); reproduzir com screenshots e resolução/browser antes de implementar. Corrigir sem regredir mobile/tablet, temas ou lightbox. |
