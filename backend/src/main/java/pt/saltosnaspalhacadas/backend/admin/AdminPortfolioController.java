...
public class AdminPortfolioController {
    ...
    ProfileResponse createProfile(@Valid @RequestBody CreateProfileRequest request) {
        ...
        return ProfileResponse.from(profiles.save(new Profile(request.slug(), request.name(), request.role(), request.description(), request.profileImageUrl(), request.profileImagePosition())));
    }
    ...
    @DeleteMapping("/profiles/{slug}/portfolio/{itemId}") @ResponseStatus(HttpStatus.NO_CONTENT)
    void deletePortfolioItem(@PathVariable String slug, @PathVariable Long itemId) {
        PortfolioItem item = items.findByIdAndProfileSlug(itemId, slug)
                .orElseThrow(() -> new org.springframework.web.server.ResponseStatusException(HttpStatus.NOT_FOUND, "Conteúdo não encontrado"));
        items.delete(item);
    }
    ...
    record CreateProfileRequest(@NotBlank @Pattern(regexp="[a-z0-9]+(?:-[a-z0-9]+)*") String slug, @NotBlank @Size(max=120) String name, @NotBlank @Size(max=120) String role, @NotBlank @Size(max=500) String description, @Size(max=2048) String profileImageUrl, @Pattern(regexp="(?:100|[0-9]{1,2})% (?:100|[0-9]{1,2})%") String profileImagePosition) { }
